...
#define PMAX 100 /*numero maximo de puntos para poligono*/
...
outtextxy(530,50,"y");
	oc=num;
	oc--;
for(ini=num;ini>0;ini--){
	setcolor(YELLOW);
	itoa(P[num-ini][0],equis,10);
	itoa(P[num-ini][1],ye,10);
	...
	outtextxy(470,g,oie);
	setcolor(GREEN);
	outtextxy(P[num-ini][0]-15,P[num-ini][1],oie);
	oc--;
}
...

{
	...
	/* printf("Diagonales de la triangulaciÂ¢n : \n");*/
	 poly[cont]=P[0][0];
	 poly[cont+1]=P[0][1];
	 setcolor(BLUE);
	 mocultar();
